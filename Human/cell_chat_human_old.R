...
Old<- subset(x=human, subset = Type == "Old")

data.input <- Old[["RNA"]]$data # normalized data matrix`
labels <- Idents(Old)
meta <- data.frame(labels = labels, row.names = names(labels))

cellchat <- createCellChat(object = Old, group.by = "ident", assay = "RNA")
...
saveRDS(cellchat, file = "cellchat_human_old.rds")
